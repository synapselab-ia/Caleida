DO $$
BEGIN
  IF to_regclass('caleida_access.invitations') IS NULL
     OR to_regclass('caleida_access.invitation_uses') IS NULL
     OR to_regclass('caleida_access.access_requests') IS NULL
     OR to_regclass('caleida_audit.entry_events') IS NULL THEN
    RAISE EXCEPTION 'schema de entrada controlada incompleto';
  END IF;
  ...
  IF NOT caleida_access.is_valid_email('pessoa@example.com')
     OR caleida_access.is_valid_email('Pessoa@Example.COM') THEN
    RAISE EXCEPTION 'contrato de e-mail normalizado divergente';
  END IF;
END;
$$;
...
BEGIN
  BEGIN
    INSERT INTO caleida_access.invitations (
      token_digest, kind, max_uses, expires_at, created_by_auth_user_id
    ) VALUES (
      'digest-invalido', 'unico', 1, CURRENT_TIMESTAMP + interval '1 hour',
  ...
  BEGIN
    INSERT INTO caleida_access.invitations (
      ...
      repeat('f', 64), 'reutilizavel', 1, CURRENT_TIMESTAMP + interval '1 hour',
      '00000000-0000-4000-8000-000000000101'
    );
...
BEGIN
  INSERT INTO caleida_access.invitations (
    ...
    repeat('a', 64), 'unico', 1, CURRENT_TIMESTAMP + interval '1 hour',
    '00000000-0000-4000-8000-000000000101'
  ...
  SELECT state, use_count INTO current_state, current_use_count
...
BEGIN
  INSERT INTO caleida_access.invitations (
    ...
    repeat('b', 64), 'reutilizavel', 2, CURRENT_TIMESTAMP + interval '1 hour',
    '00000000-0000-4000-8000-000000000101'
  ) RETURNING id INTO current_invitation_id;
  ...
  SELECT count(*) INTO use_rows
  FROM caleida_access.invitation_uses iu
  WHERE iu.invitation_id = current_invitation_id;

  IF first_count <> 1 OR second_count <> 1
     OR current_state <> 'utilizado'
     OR current_use_count <> 2
     OR use_rows <> 2 THEN
...
BEGIN
  INSERT INTO caleida_access.invitations (
    token_digest, kind, recipient_email, max_uses, expires_at, created_by_auth_user_id
  ) VALUES (
    repeat('c', 64), 'unico', 'destino@example.com', 1,
...
BEGIN
  INSERT INTO caleida_access.invitations (
    token_digest, kind, state, max_uses, expires_at,
    created_by_auth_user_id, sent_at
  ) VALUES (
    repeat('d', 64), 'unico', 'enviado', 1,
...
BEGIN
  INSERT INTO caleida_access.invitations (
    ...
    repeat('e', 64), 'unico', 1, CURRENT_TIMESTAMP + interval '1 hour',
    '00000000-0000-4000-8000-000000000101'
  ) RETURNING id INTO current_invitation_id;
...
BEGIN
  ...
  RETURNING id INTO current_request_id;
  ...
  IF changed IS NOT TRUE OR current_state <> 'aprovada' THEN
    RAISE EXCEPTION 'solicitação não foi aprovada corretamente';
  END IF;
  ...
  INSERT INTO caleida_access.access_requests (applicant_email)
  VALUES ('espera@example.com')
  RETURNING id INTO replacement_request_id;

  IF replacement_request_id = current_request_id THEN
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM caleida_audit.entry_events
